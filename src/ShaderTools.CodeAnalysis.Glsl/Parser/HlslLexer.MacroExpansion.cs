...
{
    public partial class GlslLexer
    {
        ...
        private bool TryExpandMacro(SyntaxToken token, IMacroExpansionLexer lexer, out List<SyntaxToken> expandedTokens)
        {
            ...
            switch (directive.Kind)
            {
                case SyntaxKind.FunctionLikeDefineDirectiveTrivia:
                    ...
                    if (macroArguments.Arguments.Count != functionLikeDefine.Parameters.Parameters.Count)
                    {
                        ...
                        {
                            token
                                .WithDiagnostic(Diagnostic.Create(GlslMessageProvider.Instance, token.SourceRange, (int) DiagnosticId.NotEnoughMacroParameters, token.Text))
            ...
            for (var i = 0; i < expandedTokens.Count; i++)
                ...
                {
                    var relexedToken = new GlslLexer(new SourceFile(SourceText.From(expandedTokens[i].Text), null)).Lex(LexerMode.Syntax);
        ...
        {
            private readonly GlslLexer _lexer;

            public BaseMacroExpansionLexer(GlslLexer lexer)
            {
                _lexer = lexer;
            }
