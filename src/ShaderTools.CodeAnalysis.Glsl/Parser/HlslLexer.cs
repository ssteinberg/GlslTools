...
{
    public sealed partial class GlslLexer : ILexer
    {
        ...
        public GlslLexer(SourceFile file, ParserOptions options = null, IIncludeFileSystem includeFileSystem = null)
        {
            _rootFile = file;
            ...
            if (options != null)
                ...
                {
                    var lexer = new GlslLexer(new SourceFile(
                        SourceText.From($"#define {define.Key} {define.Value}", "__ConfiguredPreprocessorDefinitions__.Glsl"),
                        null));
            ...
            List<SyntaxNode> triviaList)
        {
            ...
            if (directive.Kind == SyntaxKind.IncludeDirectiveTrivia)
            {
                ...
                try
                {
                    ...
                    if (include == null)
                    {
                        includeDirective = includeDirective.WithDiagnostic(Diagnostic.Create(GlslMessageProvider.Instance, includeDirective.SourceRange, (int) DiagnosticId.IncludeNotFound, includeFilename));
                        triviaList.Add(includeDirective);
                    }
                }
                catch (Exception ex)
                {
                    includeDirective = includeDirective.WithDiagnostic(Diagnostic.Create(GlslMessageProvider.Instance, includeDirective.SourceRange, (int) DiagnosticId.IncludeNotFound, includeFilename, ex.Message));
                    include = null;
                    triviaList.Add(includeDirective);
                }
            ...
            switch (_charReader.Current)
            {
                case '\0':
                    if (_includeStack.Count == 1 && _directives.HasUnfinishedIf())
                        _diagnostics.Add(Diagnostic.Create(GlslMessageProvider.Instance, CurrentSpanStart, (int) DiagnosticId.EndIfDirectiveExpected));
