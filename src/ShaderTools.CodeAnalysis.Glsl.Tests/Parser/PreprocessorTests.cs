...
";
        ...
        {
            const string text = @"
#line 3 ""a\path\to.Glsl""
";
            var node = Parse(text);

            TestRoundTripping(node, text);
            VerifyDirectivesSpecial(node,
                new DirectiveInfo { Kind = SyntaxKind.LineDirectiveTrivia, Status = NodeStatus.IsActive, Number = 3, Text = @"a\path\to.Glsl" });
        }
...
";
        ...
        {
            return SyntaxFactory.ParseCompilationUnit(SourceText.From(text, "__Root__.Glsl"), fileSystem);
        }
